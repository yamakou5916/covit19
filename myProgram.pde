...
void SmallCircle(int i){
  if (1000 * sin(radians(frameCnt[i])) > 0) {
    ...
    strokeWeight(3);
    ellipse(cwidth[i]-rSize[i]/3, cheight[i]-rSize[i]/3-125*cos(radians(frameCnt[i])), 80, 80);
    ...
    text(smallText0[i][0], cwidth[i]-rSize[i]/3, cheight[i]-rSize[i]/3-125*cos(radians(frameCnt[i])));
    text(smallText0[i][1], cwidth[i]+rSize[i]/3, cheight[i]-rSize[i]/3+125*sin(radians(frameCnt[i])));
    text(smallText0[i][2], cwidth[i]-rSize[i]/3, cheight[i]+rSize[i]/3+125*cos(radians(frameCnt[i])));
